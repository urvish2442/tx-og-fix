...
import SwitchComponent from "@/components/rewards/SwitchComponent";
...
const getMembershipStatus = (memberships) => {
    ...
    let status = {
        TERA: { MONTH: true, YEAR: true, LIFETIME: true },
    ...
    memberships.forEach((membership) => {
        if (membership.expiry > currentTime) {
            ...
            if (plan === "LIFETIME") {
                ...
                status[type].YEAR = false;
                status[type].MONTH = false;
            } else if (plan === "YEAR") {
                status[type].YEAR = false;
                status[type].MONTH = false;
            } else if (plan === "MONTH") {
            ...
            if (type === "YOTA") {
                status.TERA.LIFETIME = false;
                status.TERA.YEAR = false;
                status.TERA.MONTH = false;
            }
        }
    });
...
const membershipPage = () => {
    ...
    const { walletBalance } = useSelector(globalState);
    const {
        setMemberShip,
        subScribeMemberShip,
        memberShip,
    ...
    const membershipStatus = useMemo(() => {
        if (userDetails?.memberShip) {
            return getMembershipStatus(userDetails.memberShip);
        }
        return {
            TERA: { MONTH: true, YEAR: true, LIFETIME: true },
            YOTA: { MONTH: true, YEAR: true, LIFETIME: true },
    ...
    const YottaButton = () => {
        const isChainSupported = MEMBERSHIP_SUPPORTED_CHAINS.includes(chainId);
        const hasEnoughBalance = walletBalance[chainId]?.balance >= yotaPrice;
        ...
        if (yotaSubscriptionValid || !membershipStatus.YOTA[memberShip]) {
            return (
                <Button disabled style={buttonStyle}>
                    Already Subscribed
                </Button>
            ...
            <Button
                ...
                onClick={() =>
                    subScribeMemberShip({
                        ...
                        subscription: SUBSCRIPTION.YOTA,
                        price: yotaPrice,
                    })
                }
            >
                {`Unlock ${yotaPrice ? `(${yotaPrice} ETH)` : ""}`}
            </Button>
        );
    };
    ...
    const TeraButton = () => {
        const isChainSupported = MEMBERSHIP_SUPPORTED_CHAINS.includes(chainId);
        const hasEnoughBalance = walletBalance[chainId]?.balance >= teraPrice;
        ...
        if (teraSubscriptionValid || !membershipStatus.TERA[memberShip]) {
            return (
                <Button disabled style={buttonStyle}>
                    Already Subscribed
                </Button>
        ...
        return (
            ...
            >
                {`Unlock ${teraPrice ? `(${teraPrice} ETH)` : ""}`}
            </Button>
        );
    };
            ...
            <Container>
              <div className='membership-block'>
                <div className='membership-block-title'>
                  ...
                  <p>
                    Our platform provides Expiring soulbound NFTs (EIP-4671) that grant access to exclusive features and
                    enhancements, and are <span>non-transferable.</span>
                  </p>
                </div>
                <div className='membership-switch-main'>
                  {Object.keys(MEMBERSHIP).map((key, index) => (
                    <SwitchComponent
                      ...
                      checked={memberShip}
                      onChange={() => handleSwitchChange(MEMBERSHIP[key])}
                    />
                  ))}
                </div>
                <div className='membership-block-inner'>
                  <div className='membership-block-top'>
                    <div className='switch-block-inner'>
                      {/* <Switch onChange={this.handleChange} /> */}
                      {/* <span>Switch with default style</span> */}
                    </div>
                  </div>
                  <div className='membership-block-middle-block'>
                    <div className='membership-block-middle'>
                      <div className='membership-block-middle-inner'>
                        <div className='membership-img'>
                          <img src={'../../images/mega-membership.png'} alt='membership-img'></img>
                        </div>
                        <h2>Mega</h2>
                        <ul>
                          <li>1.5% Marketplace Fees</li>
                    ...
                    <div className='membership-block-middle'>
                      <div className='membership-block-middle-inner'>
                        <div className='membership-img'>
                          <img src={'../../images/tera-membership.png'} alt='membership-img'></img>
                        </div>
                        <h2>TERA</h2>
                        <ul>
                          <li className='mega-block-highlight'>MEGA +</li>
                          <li>10% Store Discount</li>
                          <li>Premium Support</li>
                          <li>Message Owners</li>
                          <li>Gated VIP Chat Room</li>
                          <li>Notifications</li>
                          <li>Generate Ai Images</li>
                          <li>Additional Customization Features </li>
                        </ul>
                        {/* {!teraSubscriptionValid && ( */}
                        <div className='btn-load-more mt-5'>
                          <TeraButton />
                        </div>
                        {/* )} */}
                      </div>
                    ...
                    <div className='membership-block-middle'>
                      <div className='membership-block-middle-inner'>
                        <div className='membership-img'>
                          <img src={'../../images/yotta-membership.png'} alt='membership-img'></img>
                        </div>
                        <h2>YOTTA</h2>
                        <ul>
                          <li className='mega-block-highlight'>TERA +</li>
                          <li>15% Store Discount</li>
                          <li>Early Access To Featured Drops</li>
                          <li>Custom Domain</li>
                          <li>Pro Analytics</li>
                        </ul>
                        {/* {!yotaSubscriptionValid && ( */}
